...
```yaml
repository_id: 01jz3k6g8h9m2n4p5q7r8s9tuv
aliases:
  cli: clients
  inv: invoices
  tpl: templates
...
A stable repository identifier (ULID) persisted in `frontdoc.yaml`.

- Generated by `frontdoc init`.
- If missing when loading an existing repository, frontdoc auto-generates one
  and writes it back to `frontdoc.yaml`.
- The value is repository-scoped and remains stable across clones/worktrees
  unless the file is explicitly edited.
